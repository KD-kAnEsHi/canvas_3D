...
def train(dataloader, model,loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        #compure predicted error
        pred = model(X)
        loss = loss_fn(pred, y)

        # back propagration
        loss.backward()         # calc gradient
        optimizer.step()        # update the weight uisng gradient
...
def test(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        ...
        correct /= size                 # accuray = correc pred/ total pred
...
# Saving Models, on computer without having to retrain them laters
...
with torch.no_grad():
    ...
    print(f'[Predictedted: "{predicted}", Actual: "{actual}"]')
